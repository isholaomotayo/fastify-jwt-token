...
function fastifyJwt(fastify, options, next) {
  ...
  if (
    ...
    return next(
      new Error(
        "RSA Signatures set as Algorithm in the options require a private and public key to be set as the secret"
      )
    );
  }
  if (
    ...
    return next(
      new Error(
        "ECDSA Signatures set as Algorithm in the options require a private and public key to be set as the secret"
      )
    );
  }
  ...
  function requestVerify(token = null, options, next) {
    ...
    if (token === null && request.headers && request.headers.authorization) {
      ...
      return next(
        new Unauthorized(messagesOptions.noAuthorizationInHeaderMessage)
      );
    }
        ...
        function verify(secretOrPublicKey, callback) {
          jwt.verify(token, secretOrPublicKey, options, (err, result) => {
            if (err instanceof jwt.TokenExpiredError) {
              return callback(
                new Unauthorized(
                  messagesOptions.authorizationTokenExpiredMessage
                )
              );
            }
            if (err instanceof jwt.JsonWebTokenError) {
              return callback(
                new Unauthorized(
                  typeof messagesOptions.authorizationTokenInvalid ===
                  "function"
                    ? messagesOptions.authorizationTokenInvalid(err)
        ...
        function checkIfIsTrusted(result, callback) {
          if (!trusted) {
            ...
            if (maybePromise && maybePromise.then) {
              maybePromise.then(trusted =>
                trusted
                  ? callback(null, result)
                  : callback(
                      new Unauthorized(
                        messagesOptions.authorizationTokenUntrusted
                      )
                    )
              );
              ...
              callback(
                new Unauthorized(messagesOptions.authorizationTokenUntrusted)
              );
            }
          }
        }
